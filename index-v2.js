...
async function handleEvent(event) {
  ...
  // テーマ選択中の処理
  const themeState = themeSelector.getThemeSelectionState(userId);
  if (themeState.isSelecting) {
    return handleThemeSelection(event, userId, userMessage, themeState.type, profile.displayName);
  }
...
初回は無料で3カード占いができるから、下のメニューから「一般占い」を選んでね🎶`;
  ...
  // メニュー選択の処理
  if (userMessage === '一般占い') {
    return handleGeneralReading(event, userId, profile.displayName);
  }
...
// 一般占い
async function handleGeneralReading(event, userId, displayName) {
  // 利用制限チェック
  ...
  // テーマ選択画面を表示
  themeSelector.setThemeSelectionState(userId, 'general');
  const themeMessage = themeSelector.getGeneralThemeSelectionMessage();
...
// テーマ選択の処理
async function handleThemeSelection(event, userId, themeNumber, type, displayName) {
  const theme = themeSelector.selectTheme(userId, themeNumber, type);
  
  if (!theme) {
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: '無効な番号です。もう一度選んでください。'
    });
  }
  ...
  // 占い結果のメッセージを作成
  const themePrefix = themeSelector.getThemePrefix(theme, type);
  let resultMessage = `🔮 ${displayName}さんの占い結果 🔮\n\n`;
  resultMessage += themePrefix;
  
  const positions = type === 'general' 
    ? ['過去', '現在', '未来']
    : ['現状', '課題', '未来'];
  ...
  // テーマ選択状態をクリア
  themeSelector.clearThemeSelectionState(userId);
...
async function handleLoveReading(event, userId, displayName) {
  ...
  // テーマ選択画面を表示
  themeSelector.setThemeSelectionState(userId, 'love');
  const themeMessage = themeSelector.getLoveThemeSelectionMessage();
